...
if (!class_exists('MELIBU_PLUGIN_SHARE_ABSTRACT')) {
    ...
    abstract class MELIBU_PLUGIN_SHARE_ABSTRACT {
         ...
         * @return type
         */
        public function get_count_by_url($url, $type = 'share') {

            global $wpdb;
            $return = 0;
            $table_name = $wpdb->prefix . "melibu_sss_s";
            $result = $wpdb->get_results("SELECT * FROM " . $table_name . " WHERE type='" . esc_sql($type) . "'");
            if ($result) {
                foreach ($result as $rest) {
         ...
         * @return type
         */
        public function get_count_by_social($social, $type = 'share', $perma = '') {
...
//                $arr['ip'] = $melibuPluginHelper->get_IP();
                ...
                $arr['count'] = $this->get_count_by_url($return['bitly_perma'], $arr['type']);

                $arr['shared'] = $this->get_count_by_social($arr['social'], $arr['type'], $arr['perma']);

                $wpdb->insert($wpdb->prefix . "melibu_sss_s", array(
                    'agent' => esc_attr($arr['agent']),
//                    'ip' => esc_attr($arr['ip']),
                    'social' => esc_attr($arr['social']),
                    'page' => esc_url($arr['link']),
                    'perma' => esc_url($arr['perma']),
                    'clicks' => 1,
                    'type' => esc_attr($arr['type']),
                    'time' => strtotime(get_date_from_gmt(date('Y-m-d H:i:s', time()), get_option('date_format') . ' ' . get_option('time_format')))
                ));
